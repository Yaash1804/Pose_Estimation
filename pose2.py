...
def compute_com(landmarks):
    """
    Compute the center of mass (CoM) based on given landmarks.
    Placeholder function – implement as per your method.
    """
    x_values = [point.x for point in landmarks]
    y_values = [point.y for point in landmarks]

    com_x = sum(x_values) / len(x_values)
    com_y = sum(y_values) / len(y_values)
...
def pose_detection(elbow_angles,knee_angles,waist_angles,shoulder_angles, w, h):
    ...
    for pose, thresholds in pose_thresholds.items():
        ...
        if match:
            ...
            com = compute_com(landmarks)

            if detected_pose in pose_classes["standing_poses"]:
                color = (0, 255, 0)  # Green for standing

                # Consider only lower body landmarks for bounding box
                keypoints = [28, 27, 30, 29]  # ankle_left, ankle_right, foot_left, foot_right

            elif detected_pose in pose_classes["ground_contact_poses"]:
                color = (255, 0, 0)  # Blue for ground contact

                # Expand keypoints to include more body parts (hips, knees, shoulders)
                keypoints = [28, 27, 30, 29, 16, 15, 20, 19, 24, 23, 12, 11]  
                # Added: hip_left, hip_right, shoulder_left, shoulder_right

            # Extract coordinates for selected keypoints
            ...
            if x_coords and y_coords:
                ...
                # Expand bounding box slightly for better CoM coverage
                padding_x = 0.05 * w  # 5% extra width
                padding_y = 0.05 * h  # 5% extra height
                ...
                # Stability Check: Allow tolerance for ground-contact poses
                if detected_pose in pose_classes["ground_contact_poses"]:
                    stability_margin = 0.1 * h  # 10% margin in height
                    is_stable = (min_x <= com[0] * w <= max_x) and ((min_y - stability_margin) <= com[1] * h <= (max_y + stability_margin))
                else:
                    is_stable = (min_x <= com[0] * w <= max_x) and (min_y <= com[1] * h <= max_y)

                # Draw CoM
                com_x, com_y = int(com[0] * w), int(com[1] * h)
                cv2.circle(frame, (com_x, com_y), 5, (0, 255, 255), -1)  # Yellow CoM dot
...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        pose_detection(elbow_angles,knee_angles,waist_angles,shoulder_angles,w,h)
